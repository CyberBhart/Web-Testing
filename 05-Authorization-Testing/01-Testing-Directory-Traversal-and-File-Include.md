...
Testing for Directory Traversal & File Include involves verifying that the application prevents unauthorized access to files or directories through directory traversal or file inclusion vulnerabilities. According to OWASP (WSTG-AUTHZ-01), vulnerabilities such as accessing sensitive files (e.g., `/etc/passwd`), remote file inclusion (RFI), local file inclusion (LFI), or source code disclosure can allow attackers to extract sensitive data or execute malicious code. This test focuses on evaluating input validation, path sanitization, and access controls across query parameters, cookies, and headers to ensure secure file handling.
...
- Execution of malicious code via remote or local file inclusion.
- Source code disclosure, enabling further attacks.
- Non-compliance with security standards (e.g., PCI DSS, GDPR).

This guide provides a practical, hands-on methodology for testing directory traversal and file inclusion vulnerabilities, adhering to OWASP’s WSTG-AUTHZ-01, with detailed tool setups, specific commands integrated into test steps, remediation strategies, and ethical considerations for professional penetration testing. **Ethical Note**: Obtain explicit permission for testing, as sending traversal payloads or accessing files may trigger security alerts or violate terms of service.
...
- **cURL**: Sends requests to test traversal and inclusion vulnerabilities.
  - Install on Linux:
    ```bash
...
This methodology follows OWASP’s black-box and gray-box approaches for WSTG-AUTHZ-01, focusing on testing directory traversal and file inclusion vulnerabilities across query parameters, cookies, RFI, LFI, encoding, Windows-specific paths, source code disclosure, UNC paths, fuzzing, and directory enumeration.
...
### 5. Test Path Traversal via Encoding with cURL

**Objective**: Ensure encoded traversal sequences are blocked.
...
2. Test encoded traversal:
   ...
   ```
3. Test double-encoded traversal:
   ```bash
   curl -i "http://example.com/getUserProfile.jsp?item=%252e%252e%252f%252e%252e%252fetc%252fpasswd"
   ```
4. Analyze responses; expected secure response rejects encoded sequences.
...
3. Fuzz the parameter with traversal payloads:
   ```bash
   Intruder -> Payloads -> Add payloads (e.g., ../../../../etc/passwd, %2e%2e%2f%2e%2e%2fetc%2fpasswd) -> Start Attack -> Check Response
   ```
